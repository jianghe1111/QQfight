#!/usr/bin/env python3
"""Batch convert NPY motion files into CSV format compatible with csv_to_npz.py."""
...
REQUIRED_KEYS = ("root_trans", "root_ori", "dof_pos")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Batch convert NPY motions in a folder to CSV format."
    )
    parser.add_argument(
        "--input_dir",
        type=str,
        required=True,
        help="Folder containing NPY motion files.",
    )
    parser.add_argument(
        "--file_pattern",
        type=str,
        default="*.npy",
        help="Glob pattern to match input files (default: *.npy).",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
...
def find_motion_sequences(data: object, prefix: str = "") -> List[Tuple[str | None, dict]]:
    ...
    if isinstance(data, dict):
        ...
        if data.shape == ():
            sequences.extend(find_motion_sequences(data.item(), prefix))
        else:
...
def motion_to_array(motion: dict) -> np.ndarray:
    """Stack root translation, root orientation, and joint DOF positions into rows."""
    root_pos = np.asarray(motion["root_trans"], dtype=np.float32)
    root_rot = np.asarray(motion["root_ori"], dtype=np.float32)
    dof_pos = np.asarray(motion["dof_pos"], dtype=np.float32)

    if not (root_pos.shape[0] == root_rot.shape[0] == dof_pos.shape[0]):
        raise ValueError(
            f"Frame count mismatch: root_trans {root_pos.shape}, root_ori {root_rot.shape}, dof_pos {dof_pos.shape}"
        )

    if root_pos.shape[1] != 3 or root_rot.shape[1] != 4:
        raise ValueError(
            f"Unexpected root shapes: root_trans {root_pos.shape}, root_ori {root_rot.shape}"
        )
...
def main() -> None:
    ...
    npy_files = sorted(input_dir.rglob(pattern))
    if not npy_files:
